...
#	-device loader,addr=0x6000000,file=DK/rootfs.cpio.gz.u-boot \
#	-nographic -serial mon:stdio -serial null \
...
#	Using my image: 2nd SMP fails because it cannot find the rootfs
#	u-boot:
...
#				--> still the same
#		changed from tmp/deploy/images/zcu102-zynqmp/core-image-minimal-zcu102-zynqmp-20170714142939.rootfs.cpio.gz.u-boot to DK/rootfs.cpio.gz
#				--> still the same
#sudo gdb --args /home/dkang/WORK/qemu-build-multi/aarch64-softmmu/qemu-system-aarch64 -machine arm-generic-fdt \
#	-device loader,file=DK/myfsb.bin,addr=0xffff0000,cpu=0 \
#	-nographic -serial mon:stdio -serial null \
#	-device loader,addr=0x6000000,file=DK/rootfs.cpio.gz \
#	-hw-dtb DK/hpsc-integrated.dtb \
#	-device loader,file=DK/bl31.elf,cpu-num=0 \
#	-device loader,addr=0x4000000,file=DK/linux-hpsc.dtb \
...
#6th trial: working
#	rootfs image must be processed by mkimage (U-Boot tool)
#	linux image must be processed by mkimage, which is done by Xilinx linux distribution
sudo gdb --args /home/dkang/WORK/qemu-build-multi/aarch64-softmmu/qemu-system-aarch64 -machine arm-generic-fdt \
	-device loader,file=DK/myfsb.bin,addr=0xffff0000,cpu=0 \
	-nographic -serial mon:stdio -serial null \
	-device loader,addr=0x6000000,file=DK/rootfs.cpio.gz.u-boot \
